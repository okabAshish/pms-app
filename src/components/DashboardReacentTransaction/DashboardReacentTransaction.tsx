...
import {OwnerDashboardTransActions} from '../../features/types';
...
const DashboardReacentTransaction = (props: Props) => {
  const arrayRearrange = (a: OwnerDashboardTransActions) => {
    let arr: Transactions = [];

    for (let i = 0; i < 3; i++) {
      arr.push({
        first_name: a[i]?.get_tanent?.first_name,
        last_name: a[i]?.get_tanent?.last_name,
        middle_name: a[i]?.get_tanent?.middle_name,
        imgUrl: a[i]?.get_tanent?.profile_photo
          ? a[i]?.get_tanent?.profile_photo
          : 'https://property.okab.ae/img/user.png',
        property_name:
          a[i]?.contract_billing_data?.contract_properties_data?.property_name,
...
DashboardReacentTransaction.defaultProps = defaultProps;

export default DashboardReacentTransaction;
